...
class MainDialog(QMainWindow,interfaceEncimaDeLaMosca.Ui_MainWindow):
	...
	def __init__(self, parent = None):
		...
		self.statusBar().showMessage(self.tr(lang_es.lang[67]))
	...
	def LoadConfig(self):
		filename = QFileDialog.getOpenFileName(self, self.tr(lang_es.lang[71]), self.dirpath)
		if filename == '':
			return False
	...
	def SaveConfig(self):
		filename = QFileDialog.getSaveFileName(self, lang_es.lang[72], self.dirpath)
		if filename == '':
			return False
	...
	def startStop(self):
		...
		if self.process_started:
			if self.iptablesCheckBox.isChecked() and  iptablesPath != '':
				self.ProxyOutput.append(lang_es.lang[73])
				...
				if self.launchDNS2Proxy.isChecked() and dns2proxyPath != '' and self.ActivateSSLSTRIP.isChecked() and self.InfectHTML.isChecked():
					...
					self.executeShortCommand(comando)
			self.LaunchButton.setText(lang_es.lang[31])
			self.process_started = False
			self.process.kill()
			self.statusBar().showMessage(self.tr(lang_es.lang[67]))
			self.ActivarCheckBoxes(True)
			self.ProxyOutput.append(lang_es.lang[75])

			if self.launchDNS2Proxy.isChecked() and dns2proxyPath != '' and self.ActivateSSLSTRIP.isChecked() and self.InfectHTML.isChecked():
			    self.dnsprocess.kill()
			    comando = 'kill `ps auxw | grep dns_hsts | grep -v grep | awk  \'{print $2}\'`'
			    self.executeShortCommand(comando)
			    self.DNSOutput.append(lang_es.lang[76])
		else:

			comando = self.ComandoOpciones()
			self.ProxyOutput.append(lang_es.lang[77]+ comando + '\n')
			try:
				self.process.start(comando)
				self.statusBar().showMessage(self.tr(lang_es.lang[78]))
				self.ActivarCheckBoxes(False)
				self.LaunchButton.setText(lang_es.lang[79])
				...
				if self.launchDNS2Proxy.isChecked() and dns2proxyPath != '' and self.ActivateSSLSTRIP.isChecked() and self.InfectHTML.isChecked():
					...
					self.DNSOutput.append(lang_es.lang[77]+ comando + '\n')
					self.dnsprocess.start(comando)

				# empezar iptables
				if self.iptablesCheckBox.isChecked() and  iptablesPath != '':
					self.ProxyOutput.append(lang_es.lang[80])
					comando = 'sysctl net.ipv4.ip_forward=1'
					self.executeShortCommand(comando)
					comando = '%s -D OUTPUT -p icmp -j DROP' % iptablesPath
					self.executeShortCommand(comando)
	...
	def ComandoOpciones(self):
		self.puerto = int(self.TCPportText.toPlainText())
	...
	def SelectFile2Text(self):
		filename = QFileDialog.getOpenFileName(self, lang_es.lang[81], './')
	...
	def SelectDir2Text(self):
		filename = QFileDialog.getExistingDirectory(self, lang_es.lang[82], './')
	...
	def SaveLog(self, filename = None):
		if filename is None:
			filename = QFileDialog.getSaveFileName(self, lang_es.lang[81], self.dirpath)
	...
	def LoadPeConfig(self, data):
		if data.find('"launcher"') == -1 or data.find('"output"') == -1 or data.find('"malware"') == -1 :
			self.Error(self.tr(lang_es.lang[83]))
			return False
		try:	
	...
	def SavePeConfig(self, filename = None):
		...
		if filename is None:
			filename = QFileDialog.getSaveFileName(self, self.tr(lang_es.lang[71]), self.dirpath)
